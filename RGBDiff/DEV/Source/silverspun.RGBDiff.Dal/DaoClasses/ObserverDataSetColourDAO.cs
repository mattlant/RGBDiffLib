...
namespace silverspun.RGBDiff.Dal.DaoClasses
{
	...
	/// <summary>
	/// General DAO class for the ObserverDataSetColour Entity. It will perform database oriented actions for
	/// a entity of type 'ObserverDataSetColourEntity'. This DAO works on an EntityFields object. 
	/// </summary>
	public partial class ObserverDataSetColourDAO : DaoBase
	{
		/// <summary>CTor</summary>
		public ObserverDataSetColourDAO() : base(InheritanceInfoProviderSingleton.GetInstance(), new DynamicQueryEngine(), InheritanceHierarchyType.None, "ObserverDataSetColourEntity", new ObserverDataSetColourEntityFactory())
		{
		}
		...
		internal ObserverDataSetColourDAO(IInheritanceInfoProvider inheritanceInfoProviderToUse, DynamicQueryEngineBase dqeToUse, InheritanceHierarchyType typeOfInheritance, string entityName, IEntityFactory entityFactory) : base(inheritanceInfoProviderToUse, dqeToUse, typeOfInheritance, entityName, entityFactory)
		{
		}
		...
		/// <summary>
		/// Retrieves entities of the type 'ObserverDataSetColourEntity' in a datatable which match the specified filter. 
		...
		/// <summary>
		/// Deletes from the persistent storage all 'ObserverDataSetColour' entities which have data in common
		/// with the specified related Entities. If one is omitted, that entity is not used as a filter. 
		/// </summary>
		/// <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or null if not.</param>
		/// <param name="observerDataSetInstance">ObserverDataSetEntity instance to use as a filter for the ObserverDataSetColourEntity objects to delete</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int DeleteMulti(ITransaction containingTransaction, IEntity observerDataSetInstance)
		{
			IEntityFields fields = EntityFieldsFactory.CreateEntityFieldsObject(silverspun.RGBDiff.Dal.EntityType.ObserverDataSetColourEntity);
			IPredicateExpression deleteFilter = CreateFilterUsingForeignKeys(observerDataSetInstance, fields);
		...
		/// <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or null if not.</param>
		/// <param name="observerDataSetInstance">ObserverDataSetEntity instance to use as a filter for the ObserverDataSetColourEntity objects to update</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(IEntity entityWithNewValues, ITransaction containingTransaction, IEntity observerDataSetInstance)
		{
			IEntityFields fields = EntityFieldsFactory.CreateEntityFieldsObject(silverspun.RGBDiff.Dal.EntityType.ObserverDataSetColourEntity);
			IPredicateExpression updateFilter = CreateFilterUsingForeignKeys(observerDataSetInstance, fields);
		...
		/// </summary>
		/// <param name="observerDataSetInstance">ObserverDataSetEntity instance to use as a filter for the ObserverDataSetColourEntity objects</param>
		/// <param name="fieldsToReturn">IEntityFields implementation which forms the definition of the fieldset of the target entity.</param>
		/// <returns>A ready to use PredicateExpression based on the passed in foreign key value holders.</returns>
		private IPredicateExpression CreateFilterUsingForeignKeys(IEntity observerDataSetInstance, IEntityFields fieldsToReturn)
		{
			IPredicateExpression selectFilter = new PredicateExpression();
			
			if(observerDataSetInstance != null)
			{
				selectFilter.Add(new FieldCompareValuePredicate(fieldsToReturn[(int)ObserverDataSetColourFieldIndex.ObserverDataSetUID], ComparisonOperator.Equal, ((ObserverDataSetEntity)observerDataSetInstance).UID));
			}
			return selectFilter;
		}
