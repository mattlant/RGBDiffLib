...
namespace silverspun.RGBDiff.Dal.DaoClasses
{
	...
	/// <summary>
	/// General DAO class for the ControlSetColour Entity. It will perform database oriented actions for
	/// a entity of type 'ControlSetColourEntity'. This DAO works on an EntityFields object. 
	/// </summary>
	public partial class ControlSetColourDAO : DaoBase
	{
		/// <summary>CTor</summary>
		public ControlSetColourDAO() : base(InheritanceInfoProviderSingleton.GetInstance(), new DynamicQueryEngine(), InheritanceHierarchyType.None, "ControlSetColourEntity", new ControlSetColourEntityFactory())
		{
		}
		...
		internal ControlSetColourDAO(IInheritanceInfoProvider inheritanceInfoProviderToUse, DynamicQueryEngineBase dqeToUse, InheritanceHierarchyType typeOfInheritance, string entityName, IEntityFactory entityFactory) : base(inheritanceInfoProviderToUse, dqeToUse, typeOfInheritance, entityName, entityFactory)
		{
		}
		...
		/// <summary>
		/// Retrieves entities of the type 'ControlSetColourEntity' in a datatable which match the specified filter. 
		...
		/// <summary>
		/// Deletes from the persistent storage all 'ControlSetColour' entities which have data in common
		/// with the specified related Entities. If one is omitted, that entity is not used as a filter. 
		/// </summary>
		/// <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or null if not.</param>
		/// <param name="controlSetInstance">ControlSetEntity instance to use as a filter for the ControlSetColourEntity objects to delete</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int DeleteMulti(ITransaction containingTransaction, IEntity controlSetInstance)
		{
			IEntityFields fields = EntityFieldsFactory.CreateEntityFieldsObject(silverspun.RGBDiff.Dal.EntityType.ControlSetColourEntity);
		...
		/// <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or null if not.</param>
		/// <param name="controlSetInstance">ControlSetEntity instance to use as a filter for the ControlSetColourEntity objects to update</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		...
		/// </summary>
		/// <param name="controlSetInstance">ControlSetEntity instance to use as a filter for the ControlSetColourEntity objects</param>
		...
		private IPredicateExpression CreateFilterUsingForeignKeys(IEntity controlSetInstance, IEntityFields fieldsToReturn)
		{
			...
			{
				selectFilter.Add(new FieldCompareValuePredicate(fieldsToReturn[(int)ControlSetColourFieldIndex.ControlSetUID], ComparisonOperator.Equal, ((ControlSetEntity)controlSetInstance).UID));
			}
			return selectFilter;
		}
